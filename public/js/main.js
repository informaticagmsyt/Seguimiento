...
var app = new Vue({
  ...
  methods: {
    ...
    getRangoTasa: async function () {
      var url =
        urlBase +
        "index.php/tasaactual?desde=" +
        this.formatFecha(this.desde) +
        "&hasta=" +
        this.formatFecha(this.hasta);
      ...
      var myChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            ...
            {
              label: "Monitor",
              backgroundColor: ["rgba(88, 193, 167, 0.2)"],
              borderColor: ["rgba(99, 193, 167, 1)"],
              fill: false,
    ...
    consultarListaSeguimiento: async function (e) {
      ...
      location.href =
        urlBase +
        "index.php/listaseguimiento?desde=" +
        this.formatFecha(this.desde) +
        "&hasta=" +
        this.formatFecha(this.hasta);
    },
    ...
    guardarTasa: async function () {
      ...
      try {
        const data = await axios({
          ...
          data: {
            tasa: this.tasar,
            fecha: this.formatFecha(this.registro.fecha),
            fuente: this.fuentet,
          },
        });
    ...
    regitrarSeguimiento: async function () {
      ...
      if (data.data.result) {
        ...
        swal({
          title: "¡ Error !",
          text:
            "Por Favor primero agregue la tasa de cambio para la fecha " +
            this.registro.fecha,
          icon: "warning",
          timer: 10000,
        });
